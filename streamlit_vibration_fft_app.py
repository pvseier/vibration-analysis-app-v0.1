...
st.set_page_config(page_title="Vibration Analysis with FFT", layout="wide")
...
if uploaded_file is not None:
    ...
    st.sidebar.header("Graph View Range")
    ...
    st.sidebar.header("Graph Labels")
    graph_title = st.sidebar.text_input("Graph Title", value="Filtered Velocity (DC Offset Removed)")
    legend_x = st.sidebar.text_input("Legend Label - X", value="Velocity X (in/s)")
    legend_y = st.sidebar.text_input("Legend Label - Y", value="Velocity Y (in/s)")
    legend_z = st.sidebar.text_input("Legend Label - Z", value="Velocity Z (in/s)")
    ...
    ax1.plot(df_view['Time_s'], df_view['DC_Free_Velocity_X'], label=legend_x)
    ax1.plot(df_view['Time_s'], df_view['DC_Free_Velocity_Y'], label=legend_y)
    ax1.plot(df_view['Time_s'], df_view['DC_Free_Velocity_Z'], label=legend_z)
    ...
    for axis, label in zip(['X', 'Y', 'Z'], [legend_x, legend_y, legend_z]):
    ...
    with st.expander("Download Data"):
        ...
        try:
            ...
            st.download_button("Download Excel", output.getvalue(), "processed_vibration_data.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
